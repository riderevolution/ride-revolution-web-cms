        ...
        <transition name="fade">
            <div id="admin" class="cms_dashboard" v-if="loaded">
                ...
                <section id="content">
                    ...
                    <pagination :apiRoute="res.news.path" :current="res.news.current_page" :last="res.news.last_page" />
                </section>
            </div>
        </transition>
        <transition name="fade">
    ...
    export default {
        ...
        methods: {
            ...
            fetchData (status) {
                const me = this
                me.loader(true)
                me.$axios.get(`api/web/news?enabled=${me.status}`).then(res => {
                    if (res.data) {
                        setTimeout( () => {
                            me.res = res.data
                            me.totalResults = me.res.news.total
                            me.loaded = true
                        }, 500)
                    }
                }).catch(err => {

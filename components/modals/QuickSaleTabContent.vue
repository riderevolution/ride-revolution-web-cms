<template>
    <div>
        <div :class="`modal_tab_content_wrapper ${(value.isGiftShow) ? 'no_qty' : '' }`" v-show="(value.isProductShow == true) ? $parent.toCompare.product == value.product.product_category_id && $parent.isProduct : (value.isGiftShow == true ? true : false)">
            <div class="form_check">
                <input type="checkbox" :id="`product_${value.id}_${unique}`" name="product[]" class="action_check" @change="toggleChecked(unique)" :checked="value.isChecked">
            ...
            <div class="form_group" v-if="!value.isGiftShow">
                ...
                <div class="form_flex_input">
                    <input type="text" name="quantity[]" class="default_text number" autocomplete="off" v-model="quantity" v-validate="'numeric|min_value:0'">
